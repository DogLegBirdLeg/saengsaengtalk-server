...
class StoreList(Resource):
    ...
    @inject
    def get(self, store_service: StoreService = Provide[Container.store_service]):
        store_list = store_service.get_list()

        return list(map(lambda store: store.json, store_list))
...
class StoreMenuList(Resource):
    ...
    @inject
    def get(self, store_id, store_service: StoreService = Provide[Container.store_service], menu_service: MenuService = Provide[Container.menu_service]):

        store = store_service.get(store_id)
        sections = menu_service.get_summary_list(store_id)
...
class StoreMenuDetail(Resource):
    ...
    @inject
    def get(self, store_id, menu_id, menu_service: MenuService = Provide[Container.menu_service]):
